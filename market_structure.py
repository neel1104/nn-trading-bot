...
class MarketStructureAnalyzer:
    ...
    Uses N-candle color reversals to identify swing points and structure breaks (BOS).
    
    Pipeline: Analyzer(df).filter_inside_bars().identify_pivots().find_bos().plot()
    """
    ...
    def identify_pivots(self, min_candles=2) -> 'MarketStructureAnalyzer':
        ...
        The reversal condition is met when 'min_candles' of the opposite color appear in sequence.
        """
        ...
        for i in range(1, len(df)):
            ...
            if trend != 'bull' and all(df['is_green'].iloc[i-j] for j in range(min_candles)):
                if trend == 'bear':
            ...
            elif trend != 'bear' and all(df['is_red'].iloc[i-j] for j in range(min_candles)):
                if trend == 'bull':
                    search_range = df.iloc[current_trend_start_idx : i+1]
    ...
    def _add_labels(self, df):
        """
        Internal helper to assign market structure labels based on previous swing points:
    ...
    def plot(self, title: str = "Market Structure Analysis", zoom_days: int = 30, filepath: Optional[str] = None):
        ...
        if self.df_pivots is not None:
            ...
            fig.add_trace(go.Scatter(
                x=highs.index, y=highs['pivot_high'], mode='markers+text',
                text=highs['label'], textposition='top center',
                marker=dict(color='green', size=10, symbol='triangle-down'),
                name='Swing Highs', textfont=dict(color='green', size=11)
            ))
            
            fig.add_trace(go.Scatter(
                x=lows.index, y=lows['pivot_low'], mode='markers+text',
                text=lows['label'], textposition='bottom center',
                marker=dict(color='red', size=10, symbol='triangle-up'),
                name='Swing Lows', textfont=dict(color='red', size=11)
            ))
        ...
        if 'is_inside' in df.columns:
            ...
            if not inside_bars.empty:
                # Nudge the dot 0.5% above the candle high for better visibility
                fig.add_trace(go.Scatter(
                    x=inside_bars.index, y=inside_bars['High'] * 1.005, mode='markers',
